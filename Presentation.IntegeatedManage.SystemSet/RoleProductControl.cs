...
namespace FengSharp.OneCardAccess.Presentation.IntegeatedManage.SystemSet
{
    public partial class RoleProductControl : RoleProductControl_Design
    {
        ...
        private void RoleProductControl_Load(object sender, EventArgs e)
        {
            if (!this.DesignMode)
            {
                this.Facade = new RoleProductControlFacade(this);
        ...
        {
            this.Facade.DelProductTempData();
        }
        ...
        private void btnAll_Click(object sender, EventArgs e)
        {
            try
            {
                var datas = this.bindingSource1.DataSource as List<ProductInfoRightStatusTempEntity>;
        ...
        internal void BindData(ProductInfoRightStatusTempEntity[] entitys)
        {
            this.bindingSource1.DataSource = new List<ProductInfoRightStatusTempEntity>(entitys);
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            try
            {
                if (e.Clicks != 2 || this.gridView1.FocusedRowHandle < 0)
                    return;
                var entity = this.gridView1.GetRow(this.gridView1.FocusedRowHandle) as ProductInfoRightStatusTempEntity;
                if (entity.HasCate)
                {
            ...
            try
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    if (this.currentbindentity.IsSuper)
                        return;
                    var entity = gv.GetRow(e.RowHandle) as ProductInfoRightStatusTempEntity;
                    if (entity == null)
                        return;
                    bool check = !entity.Status;
                    this.Facade.SaveTempData(new ProductInfoRightStatusTempEntity[] { entity }, check);
                    entity.Status = check;
                    IsdataChanged = true;
    ...
    public class RoleProductControl_Design : TreeLevelUserControl<RoleProductControlFacade>
    {
    }
    public class RoleProductControlFacade : ActualBase<RoleProductControl>
    {
        IAccessService _AccessService = ServiceProxyFactory.Create<IAccessService>();
        IProductService _ProductService = ServiceProxyFactory.Create<IProductService>();
        public RoleProductControlFacade(RoleProductControl actual)
            : base(actual) { }
        ...
        internal void NewBindData(RoleEntity roleEntity)
        {
            ...
            ProductInfoRightStatusTempEntity[] entitys =
                _AccessService.GetNewProductBaseRight(roleEntity.RoleId, newguidflag, this.Actual.guidflag);
        ...
        internal void BindData(RoleEntity roleEntity, int pid)
        {
            if (roleEntity == null)
                return;
            ProductInfoRightStatusTempEntity[] entitys =
                _AccessService.GetProductBaseRight(roleEntity.RoleId, this.Actual.guidflag, pid);
            this.Actual.BindData(entitys);
            this.Actual.lastBindRoleId = roleEntity.RoleId;
        }
        ...
        internal void SaveRightData(List<ProductInfoRightStatusTempEntity> datas)
        {
            _AccessService.SaveProductRightData(datas);
        }

        internal void DelProductTempData()
        {
            _AccessService.DelProductRightTempData(this.Actual.guidflag);
        }

        internal void SaveTempData(ProductInfoRightStatusTempEntity[] productInfoRightStatusTempEntity, bool check)
        {
            if (this.Actual.CurrentBindEntity == null) throw new BusinessException("角色不可为空");
            _AccessService.SaveProductRightTempData(this.Actual.CurrentBindEntity.RoleId, productInfoRightStatusTempEntity.Select(t => t.ProductId).ToArray(), this.Actual.guidflag, check);
        }
    }
}
