...
namespace FengSharp.OneCardAccess.Presentation.IntegeatedManage.SystemSet
{
    public partial class RoleCompanyControl : RoleCompanyControl_Design
    {
        ...
        private void RoleCompanyControl_Load(object sender, EventArgs e)
        {
            if (!this.DesignMode)
            {
                this.Facade = new RoleCompanyControlFacade(this);
        ...
        {
            this.Facade.DelCompanyTempData();
        }
        ...
        private void btnAll_Click(object sender, EventArgs e)
        {
            try
            {
                var datas = this.bindingSource1.DataSource as List<CompanyInfoRightStatusTempEntity>;
        ...
        internal void BindData(CompanyInfoRightStatusTempEntity[] entitys)
        {
            this.bindingSource1.DataSource = new List<CompanyInfoRightStatusTempEntity>(entitys);
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            try
            {
                if (e.Clicks != 2 || this.gridView1.FocusedRowHandle < 0)
                    return;
                var entity = this.gridView1.GetRow(this.gridView1.FocusedRowHandle) as CompanyInfoRightStatusTempEntity;
                if (entity.HasCate)
                {
            ...
            try
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    if (this.currentbindentity.IsSuper)
                        return;
                    var entity = gv.GetRow(e.RowHandle) as CompanyInfoRightStatusTempEntity;
                    if (entity == null)
                        return;
                    bool check = !entity.Status;
                    this.Facade.SaveTempData(new CompanyInfoRightStatusTempEntity[] { entity }, check);
                    entity.Status = check;
                    IsdataChanged = true;
    ...
    public class RoleCompanyControl_Design : TreeLevelUserControl<RoleCompanyControlFacade>
    {
    }
    public class RoleCompanyControlFacade : ActualBase<RoleCompanyControl>
    {
        IAccessService _AccessService = ServiceProxyFactory.Create<IAccessService>();
        ICompanyService _CompanyService = ServiceProxyFactory.Create<ICompanyService>();
        public RoleCompanyControlFacade(RoleCompanyControl actual)
            : base(actual) { }
        internal void NewBindData(RoleEntity roleEntity)
        {
            ...
            CompanyInfoRightStatusTempEntity[] entitys =
                  _AccessService.GetNewCompanyBaseRight(roleEntity.RoleId, newguidflag, this.Actual.guidflag);
        ...
        internal void BindData(RoleEntity roleEntity, int pid)
        {
            if (roleEntity == null)
                return;
            CompanyInfoRightStatusTempEntity[] entitys =
                _AccessService.GetCompanyBaseRight(roleEntity.RoleId, this.Actual.guidflag, pid);
            this.Actual.BindData(entitys);
            this.Actual.lastBindRoleId = roleEntity.RoleId;
        }
        ...
        internal void SaveRightData(List<CompanyInfoRightStatusTempEntity> datas)
        {
            _AccessService.SaveCompanyRightData(datas);
        }

        internal void DelCompanyTempData()
        {
            _AccessService.DelCompanyRightTempData(this.Actual.guidflag);
        }

        internal void SaveTempData(CompanyInfoRightStatusTempEntity[] companyInfoRightStatusTempEntity, bool check)
        {
            if (this.Actual.CurrentBindEntity == null) throw new BusinessException("角色不可为空");
            _AccessService.SaveCompanyRightTempData(this.Actual.CurrentBindEntity.RoleId,
                companyInfoRightStatusTempEntity.Select(t => t.CompanyId).ToArray(),
