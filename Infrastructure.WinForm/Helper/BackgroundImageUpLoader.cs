...
{
    public class BackgroundImageUpLoader : IDisposable
    {
        private BackgroundImageUpLoaderEventHandler _UpLoaded;
        private BackgroundWorker worker;
        public BackgroundImageUpLoader()
        {
        }
        ...
        private void UpLoadAsyncThreadCore(object sender, DoWorkEventArgs e)
        {
            ...
            string fileuploadname = Path.GetTempFileName();
            try
            {
                this.LastImage.Save(fileuploadname);
            }
            catch (Exception ex)
            {
                ex.ToString();
                throw new Exception("图片正在占用");
            }
            // 要上传的文件
            FileStream fs = new FileStream(fileuploadname, FileMode.Open, FileAccess.Read);
            ...
            finally
            {
                fs.Close();
                r.Close();
                if (File.Exists(fileuploadname))
                    File.Delete(fileuploadname);
            }
        }
